...
class ConvertVideo:
    def __init__(self, input_video_path, output, model_path, device):
        ...
        if ort.get_device() == 'GPU' and self.device == "gpu":
            self.sess_land = ort.InferenceSession(self.model_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider', ])
        else:
            self.sess_land = ort.InferenceSession(self.model_path, providers=['CPUExecutionProvider', ])
    ...
    def __call__(self):
        # load video
        vid = Videocap(self.input_video_path, self.name)
        codec = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
        num = vid.total
        self.video_out = cv2.VideoWriter(os.path.join(self.output, os.path.basename(self.input_video_path).rsplit('.', 1)[0] + f'_{self.name}.mp4'), codec, vid.fps,
                                         (vid.ori_width, vid.ori_height))
        pbar = tqdm(total=vid.total, )
        pbar.set_description(
            f"Running: {os.path.basename(self.input_video_path).rsplit('.', 1)[0] + f'_{self.name}.mp4'}")
        while num > 0:
        ...
        pbar.close()
        self.video_out.release()
        return os.path.join(self.output, os.path.basename(self.input_video_path).rsplit('.', 1)[0] + f'_{self.name}.mp4')
...
if __name__ == '__main__':
    ...
    func = ConvertVideo(input_video_path, model_path, output, device='cpu')
    info = func()
